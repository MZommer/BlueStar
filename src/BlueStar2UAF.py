...
def fileDialog(title: str, type: str) -> None:
    path = filedialog.askopenfilename(title=title, filetypes=((f"{type.upper()} files", f"*.{type.lower()}"), ("All Files", ".*")))
...
def main():
    ...
    ### Save the UAF ###
    json.dump(song.tml_dance, open(f"output/{MainJson['MapName']}/timeline/{mapname}_tml_dance.dtape.ckd", "w", encoding="utf-8"), ensure_ascii=False)
    json.dump(song.tml_karaoke, open(f"output/{MainJson['MapName']}/timeline/{mapname}_tml_karaoke.ktape.ckd", "w", encoding="utf-8"), ensure_ascii=False)
    json.dump(song.mainsequence, open(f"output/{MainJson['MapName']}/cinematics/{mapname}_mainsequence.tape.ckd", "w", encoding="utf-8"), ensure_ascii=False)
    json.dump(song.musictrack, open(f"output/{MainJson['MapName']}/audio/{mapname}_musictrack.tpl.ckd", "w", encoding="utf-8"), ensure_ascii=False)
    for index, amb in enumerate(song.ambtpls):
        os.makedirs(f"output/{MainJson['MapName']}/audio/amb", exist_ok=True)
        json.dump(amb, open(f"output/{MainJson['MapName']}/audio/amb/amb_{mapname}_{index}.tpl.ckd", "w", encoding="utf-8"), ensure_ascii=False)
    json.dump(song.songdesc, open(f"output/{MainJson['MapName']}/songdesc.tpl.ckd", "w", encoding="utf-8"), ensure_ascii=False)
